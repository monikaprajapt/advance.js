...
axios.get(url1)
    .then(resp => {
        ...
        console.log(DATA[courses_no-1]["name"])
        URL2 = axios.get("https://api.merakilearn.org/courses/" + DATA[courses_no - 1]["id"]+" " + '/exercises')
        .then(resp => {
            let data2 = resp.data
            let n = JSON.stringify(data2, null, 4)
            fs.writeFileSync("child.json", n)
                var j = 0
                var k = 0
                var arry1 = []
                var arry2 = []
                for (i of data2["course"]["exercises"]) {
...
// fs=require("fs")
// var axios=require('axios')
// // axios.get('https://api.merakilearn/courses')
...
//       console.log(response);
//       fs.writeFilesync("j.tex",JSON.stringify(response,null,4))
//     } catch (error) {
//       console.error(error);
//     }
//   }
